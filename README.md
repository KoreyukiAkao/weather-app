# 🌤 Weather App (React + Vite + TypeScript) 開発メモ

## Weather App 仕様定義

### 1. アプリケーション概要

Weather App は、ユーザーが入力した都市の天気情報を表示する Web アプリケーションです。ユーザーインターフェースはシンプルで、指定した都市の天気、気温、天気アイコンなどを視覚的に表示します。

### 2. 主な機能

- ユーザーが都市名を入力できる検索ボックスを提供する。
- 入力した都市名に基づいて、**OpenWeather API**などから天気情報を取得する。
- 取得した天気データを画面に表示する。
- 天気情報には、**現在の天気**、**気温**、**湿度**、**風速**、**天気アイコン**を含む。
- エラーハンドリング: 入力された都市名が無効な場合、エラーメッセージを表示する。

### 3. UI/UX 要件

- **検索ボックス**:
  - 都市名を入力し、検索する。
  - 検索結果として、都市名、天気、気温、風速などを表示。
  - 入力欄の横に「検索」ボタンを配置。
- **天気情報表示**:
  - 天気アイコン、気温、風速、湿度などの情報を見やすいレイアウトで表示。
  - レスポンシブデザインを採用し、モバイルでも使いやすくする。
- **エラーメッセージ**:
  - 都市が見つからない、または入力が無効な場合は適切なエラーメッセージを表示。

### 4. 技術要件

- **フロントエンド**:
  - React.js を使用してコンポーネントベースでアプリケーションを構築。
  - TypeScript を使用し、型安全を確保。
  - アニメーションや動的な要素は、**GSAP**などのライブラリで簡単に追加。
- **API**:
  - **OpenWeather API**などを使用して天気データを取得。
  - ユーザーが都市名を入力すると、API からその都市の天気データを取得する。
...
- **フロントエンド**:
  ...
  - React.js
  - TypeScript
  - CSS（SCSS や Styled-components など）
  - GSAP（アニメーションが必要な場合）
...
- **パフォーマンス**: ページの読み込み速度や API 呼び出しの応答時間を最適化。
- **アクセシビリティ**: 色のコントラストやフォントサイズを考慮し、視覚に障害のあるユーザーにも対応。
- **モバイル対応**: スマートフォンやタブレットでの利用を想定したレスポンシブデザイン。

### 7. 開発とデプロイ

- **開発環境**: Vite（ビルドツール）、Git（バージョン管理）、GitHub（リポジトリホスティング）
- **デプロイ**: Netlify や Vercel などの CI/CD サービスを使用してデプロイ。

### 8. 必須外部 API

- **OpenWeather API**: 都市ごとの天気データを取得するために使用。
  - 例: `https://api.openweathermap.org/data/2.5/weather?q={cityName}&appid={API_KEY}`

### 9. エラーハンドリング

- ユーザーが存在しない都市名を入力した場合や、API の呼び出しに失敗した場合にエラーメッセージを表示。
- ネットワークエラーが発生した場合にも適切なエラー表示を行う。

### 10. ユーザーインターフェース（UI）要素
